...
const LANGUAGES: GitHubLanguage[] = [
  "C", "C++", "CSS", "Elixir", "Go", "Haskell", "HTML", "Java",
  "JavaScript", "Kotlin", "Lua", "OCaml", "Python", "Rust",
  "Shell", "Svelte", "TypeScript", "Vue", "Zig",
];
...
async function fetchWithRetry(url: string): Promise<string> {
  ...
  throw new Error(`Failed to fetch ${url} after 5 attempts. Last error: ${lastError?.message ?? "Unknown error"}`);
}

async function scrapeLanguage(language: GitHubLanguage): Promise<ScrapingResult> {
  console.log(`info: scraping ${language}`);

  try {
    const html = await fetchWithRetry(`https://github.com/trending/${GitHubLanguages[language]}`);
    ...
    const repositories: Repository[] = root
      ...
      .flatMap((row, index) => {
        ...
        const statLinks = row.querySelectorAll("a.Link--muted.d-inline-block.mr-3");
        const getStat = (type: string): string =>
          Array.from(statLinks)
            .find(link => link.querySelector("svg")?.getAttribute("aria-label") === type)
            ?.text.trim() ?? "0";
...
async function scrapeLanguages(languages: GitHubLanguage[]): Promise<ScrapingResult[]> {
...
function formatToMarkdown(results: ScrapingResult[], date: string): string {
  const createAnchor = (text: string): string => text.toLowerCase().replace(/\W/g, "-");
  ...
  return `# GitHub Trending - ${date}\n\n## Table of Contents\n\n${
    results.map(r => `- [${r.language}](#${createAnchor(r.language)})`).join("\n")
...
function findProjectRoot(path: string = process.cwd()): string {
  return existsSync(join(path, ".git"))
    ? path
    : findProjectRoot(dirname(path));
}

function getOutputPath(date: string): string {
  const outputDir = join(findProjectRoot(), "data", date.slice(0, 4), date.slice(5, 7));
...
async function main() {
  ...
  console.log(`info: completed ${successful.length}/${results.length} languages`);
