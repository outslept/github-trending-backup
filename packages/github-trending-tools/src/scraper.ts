...
import { type GitHubLanguage, getLanguageUrlParam } from './languages';

export async function scrapeLanguageWithRetry(language: GitHubLanguage, maxRetries = 3): Promise<string> {
...
export async function scrapeLanguage(language: GitHubLanguage): Promise<string> {
  let result = `## ${language}\n\n`;
  const urlParam = getLanguageUrlParam(language);
