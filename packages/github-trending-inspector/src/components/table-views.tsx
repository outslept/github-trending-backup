...
function StatItem({ icon: Icon, value }: { icon: typeof Star; value: number | null }) {
  const isPos = value != null && value > 0;
  return (
    <span className="flex items-center gap-1">
      <Icon className={cn('size-3', isPos ? 'text-emerald-500' : 'text-muted-foreground')} />
      <span
        className={cn(
          'font-mono text-sm tabular-nums',
          isPos ? 'font-medium text-emerald-600 dark:text-emerald-400' : 'text-muted-foreground',
        )}
      >
  ...
  return (
    ...
    >
      <div className="flex min-w-0 items-center gap-3">
        <span className="font-mono text-sm tabular-nums text-muted-foreground">#{repo.rank}</span>
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-12 text-muted-foreground">
      ...
      <div className="text-center">
        <p className="mb-1 text-sm font-medium text-foreground">no repositories found</p>
    ...
    <div>
      <TableComponent>
        ...
        <TableBody>
          {rows.length > 0 ? (
            rows.map((row) => (
              <TableRow key={row.id} className="border-border/40 odd:bg-muted/5">
